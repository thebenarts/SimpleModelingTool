...
void main(){
	...
	p0_3d = modelViewProjMatrix * p0_3d;			// Compute the vertex position in the usual fashion. 
	vec2 p0 = p0_3d.xy / p0_3d.w;								// 2D position
 
	p1_3d = modelViewProjMatrix * p1_3d;			// Compute the vertex position in the usual fashion. 
	vec2 p1 = p1_3d.xy / p1_3d.w;								// 2D position
 
	p2_3d = modelViewProjMatrix * p2_3d;			// Compute the vertex position in the usual fashion. 
	vec2 p2 = p2_3d.xy / p2_3d.w;								// 2D position
	...
	float area0 = abs(v10.x*v20.y - v10.y*v20.x);					// Compute 2D area of triangle.
 
	float h0 = area0/length(v10-v20);								// Compute distance from vertex to line in 2D coords
 
	dist = vec3(h0, 0.0, 0.0);
 
	dist *= p0_3d.w;												// Quick fix to defy perspective correction
